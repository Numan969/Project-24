...
import usersModal from './models/usersModal.js';
...
// Function to create a verified admin user
const createAdminUser = async () => {
  try {
    const existingAdmin = await usersModal.findOne({ email: "admin@example.com", isAdmin: true });
    if (existingAdmin) {
      console.log("Admin already exists.");
    } else {
      const salt = await bcrypt.genSalt(10);
      const hashedPassword = await bcrypt.hash("adminpassword", salt); // Set a secure default password
      const adminUser = new usersModal({
        name: "Admin User",
